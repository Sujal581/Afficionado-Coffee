...
with col1:
    ...
    fig.add_trace(go.Bar(
        ...
        text=comparison["CV Score (%)"].apply(lambda x: f"{x:.1f}%"),
        textposition="outside",
...
insight_card(f"{best_cv[0]} leads in CV score ({best_cv[1]:.2f}%) — strong cross-validation performance means it generalises well to unseen data.", kind="success")
