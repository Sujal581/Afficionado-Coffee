...
with col1:
    ...
    weekly_sorted = weekly.sort_values("weekday")
...
with col2:
    ...
    monthly["month_name"] = monthly["month"].map(month_name)
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            size=10,
            color=[COLORS["caramel"] if m == peak_month else COLORS["espresso"] for m in monthly["month"]],
...
with col4:
    ...
    if "quarter" in df.columns:
        ...
        fig = go.Figure(go.Bar(
            ...
            text=qtr["revenue"].apply(lambda x: f"${x/1000:.0f}k"),
            textposition="outside",
            hovertemplate="<b>%{x}</b><br>Revenue: $%{y:,.0f}<extra></extra>",
        ))
        fig.update_yaxes(tickprefix="$", tickformat=",.0f")
